...
@section('adminContent')
            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($expenses->count() > 0 && $expenses[0]->trashed())
                            {{ __('Expenses Trash') }}
                        @else
                            {{ __('Expenses') }}
                        @endif
 ...
 <form style="display: inline-block" action="">
                        ...
                        @if (auth()->user()->hasPermission('expenses-create'))
                            <a href="{{ route('expenses.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        {{-- <a href="{{ route('expenses.trashed') }}" class="btn btn-falcon-default btn-sm" type="button"><span
        ...
        <div class="card-body p-0">
            <div class="table-responsive scrollbar">
                @if ($expenses->count() > 0)
                                    ...
                                    <td class="joined align-middle py-2">
                                        @if ($expense->id != null)
                                            <button class="btn btn-sm btn-outline-success me-1 mb-1" type="button"
                                                ...
                                                data-bs-target="#show-task-{{ $expense->id }}">{{ __('show') }}
                                            </button>
                                        @endif
                                    </td>


                                    <td class="joined align-middle py-2">{{ $expense->created_at }} <br>
                                        {{ interval($expense->created_at) }} </td>
                                    @if ($expense->trashed())
                                        <td class="joined align-middle py-2">{{ $expense->deleted_at }} <br>
                                            {{ interval($expense->deleted_at) }} </td>
                                    @endif
                                        ...
                                        <div class="dropdown font-sans-serif position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0"
                                                ...
                                                <div class="bg-white py-2">
                                                    @if ($expense->trashed() && auth()->user()->hasPermission('expenses-restore'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('expenses.restore', ['expense' => $expense->id]) }}">{{ __('Restore') }}</a>
                                                    @elseif(auth()->user()->hasPermission('expenses-update'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('expenses.edit', ['expense' => $expense->id]) }}">{{ __('Edit') }}</a>
                                                    @endif
                                                    {{-- @if (auth()->user()->hasPermission('expenses-delete') || auth()->user()->hasPermission('expenses-trash'))
                                                        <form method="POST"
                                                            action="{{ route('expenses.destroy', ['expense' => $expense->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $expense->trashed() ? __('Delete') : __('Trash') }}</button>
                                                        </form>
                                                    @endif --}}
                                                </div>
                                            </div>
                    ...
                    @foreach ($expenses as $expense)
                        <div class="modal fade" id="show-task-{{ $expense->id }}" tabindex="-1" role="dialog"
                            aria-hidden="true">
                                            ...
                                            <div class="table-responsive scrollbar">
                                                <table class="table table-bordered overflow-hidden">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr class="btn-reveal-trigger">
                                                            <td>{{ __('Type') }}</td>
                                                            <td>{{ ucwords(str_replace('_', ' ', $expense->expense_type)) }}
                                                            </td>
                                                        </tr>
                    ...
                    @endforeach
                @else
                    <h3 class="p-4">{{ __('No expenses to Show') }}</h3>
                @endif
            </div>
        </div>
